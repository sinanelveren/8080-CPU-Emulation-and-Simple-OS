...
uint64_t GTUOS::handleCall(const CPU8080 &CPU){
    ...
    if(CPU.state->a == 18){
		...
		std::cout << endl << "Unsupported OS call" << endl;
		throw -1;
	}
...
/*set fileSystem file*/
void GTUOS::setFileSystem(char fileSystemName[]) {

    blockNumber = 0;       //first file handle begining


    fd_fileSystem = open (fileSystemName, O_RDWR | O_CREAT | O_TRUNC , 0666);
    ...
    char temp[MAX_BLOCK_SIZE * sizeof(FileTable)];
    memset(&temp, '.', sizeof(temp));
    write(fd_fileSystem, &temp, sizeof(temp));

    //   close(fd_fileSystem);

   // fd_fileSystem = open (fileSystemName,O_RDWR);

}


/*handle of system call CreateFile*/
uint64_t GTUOS::FileCreate(const CPU8080 & CPU) {                          //ok
    ...
    for (; ; ++i) {
        if( (char)CPU.memory->at(offset) == '\0' || (char)CPU.memory->at(offset) == '\0' )  //until end of string or line
            break;
        fileName[i] = (char) CPU.memory->at(offset);                      //get the string chararter by character
        offset++;                                                         //next character
    }                                                                     //assign null character to know end of string

    fileName [i] = '\0';
    if (i < 1)      //size < 1
        CPU.state->b = 0;               //fail
    else{
        if (lseek(fd_fileSystem, 256 * (blockNumber++), SEEK_SET) == (off_t) - 1) {         //seek block size * new block
            perror("lseek error");
            exit(1);
        }
        ...
        char timeAcc[32];
        char timeMod[32];
        strcpy(timeAcc, asctime(localtime(&result)));
        timeAcc[strlen(timeAcc) - 1] = 0;
       // strcpy(timeAcc,  put_time(localtime(&now), "%F %T"));

        memset(&fileTable, '.', sizeof(fileTable));

        if(fileName[strlen(fileName)-1] == '\n')
            fileName[strlen(fileName)-1]=0;

        strcpy(fileTable.name, fileName);
        fileTable.block = blockNumber-1;
        strcpy(fileTable.lastAccess, timeAcc);
        strcpy(fileTable.lastModification,  timeAcc);
        strcpy(fileTable.content, "");
        fileTable.size = 0;
        fileTable.seek = 0;


        write(fd_fileSystem, &fileTable, sizeof(fileTable));


        fileTableList.push_back(fileTable);
    }
...
/*handle of system call fileOpen*/
uint64_t GTUOS::FileOpen(const CPU8080 & CPU) {                          //ok
    ...
    if (i < 1)                          //size < 1
        ...
        for (i = 0; i < fileTableList.size() ; ++i) {

            if ( strcmp( fileTableList.at(i).name, fileName ) == 0 ){

                blockNum = fileTableList.at(i).block;
                break;
            }
        }
        ...
        if (blockNum == -1){
            ...
            CPU.state->b = blockNum +1;               //succes
           // cout << fileName  << "*found*" <<(int)CPU.state->b ;
        }
    }
}
...
/*handle of system call fileClse*/
uint64_t GTUOS::FileClose(const CPU8080 & CPU) {
    int blockNum = CPU.state->b -1;
    int isClosed = 0;

    for (vector<FileTable>::iterator it = fileTableList.begin(); it != fileTableList.end(); ) {
        if ( it.base()->block == blockNum ){
            fileTableList.erase(it);
            isClosed = 1;
            break;
        } else {
            ++it;
...
/*handle of system call FileRead*/
uint64_t GTUOS::FileRead(const CPU8080 & CPU) {
    ...
    numBytes = CPU.state->d;
    blockNum = CPU.state->e - 1;

    if (lseek(fd_fileSystem, 256 * blockNum, SEEK_SET) == (off_t) -1) {         //fd: seek block size * new block
        perror("lseek error");
        exit(1);
    }
    ...
    read(fd_fileSystem, &fileTable, sizeof(fileTable));


    char buffer[numBytes];
    //  memcpy(buffer, fileTable.content, strlen());
    // strncpy ( buffer, fileTable.content, numBytes );

    int j = 0;

    for (int i = fileTable.seek; i < numBytes && i < strlen(fileTable.content); ++i) {
        buffer[j++] = fileTable.content[i];
        CPU.memory->at(offset++) = fileTable.content[i];                                     //load the input value to memory shown offset

    }
    buffer[j] = '\0';


    CPU.state->b = j;           // j bytes is read
...
/*handle of system call FileWrite*/
uint64_t GTUOS::FileWrite(const CPU8080 & CPU) {


    char     buffer[148];       /*max buffer*/
    uint16_t offset = (CPU.state->b << 8) | (CPU.state->c & 0xff);  //convert to 16 bit for find the offset
    int      i = 0;
    int      blockNum = -1;
    int      numBytes = 0;
    ...
    for (; i < numBytes ; ++i) {
        ...
        buffer[i] = (char) CPU.memory->at(offset);                      //get the string chararter by character
        offset++;                                                         //next character
    }                                                                     //assign null character to know end of string
    ...
    if (i < 1)                          //size < 1
        ...
        int j = 0;
        offset = (CPU.state->b << 8) | (CPU.state->c & 0xff);  //convert to 16 bit for find the offset
        for (i = fileTable.seek; i < numBytes*2 && i < sizeof(fileTable.content); ++i) {
            sprintf(&(fileTable.content[i++]), "%02X ",  CPU.memory->at(offset++));
        ...
        CPU.state->b = j;           // j bytes is write
        ...
        strcpy(fileTable.lastAccess, timeAcc);
        strcpy(fileTable.lastModification,  timeAcc);
        fileTable.size = strlen(fileTable.content);
...
/*handle of system call FileSeek*/
...
void GTUOS::printFileTable(const CPU8080 & CPU) {

    cout << endl << "\t\tFile Table Info" << endl;
    cout << "Block \tName \t\tLastAccess \t\t\tLastModifi \t\tSIZE \tSEEK";
    for (int i = 0; i < fileTableList.size(); ++i) {
        cout << endl << fileTableList.at(i).block << " \t"
             << fileTableList.at(i).name  << " \t"
             << fileTableList.at(i).lastAccess  << " \t"
             << fileTableList.at(i).lastModification  << " \t"
             << fileTableList.at(i).size  << " \t"
             << fileTableList.at(i).seek;
    }
